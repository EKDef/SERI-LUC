...
compare_all_skills <- function(data, reference_soc_code) {
  ...
  result <- result %>%
    mutate(
      ...
      `3 digit SOC 2020 code` = substr(as.character(`SOC 2020 code`), 1, 3)
    )
  
  # Calculate the average skill counts for each 3-digit SOC code
  result <- result %>%
    group_by(`3 digit SOC 2020 code`) %>%
    mutate(
...
```{r}
# Sort the result by avg_equal_or_lower_skill_count in descending order and select the top 10 excluding the first entry (first entry will likely be the same SOC as target)
top_10_soc_group <- compare_all_skills(ons_occ_skills, 5111) %>%
  arrange(desc(avg_equal_or_lower_skill_count)) %>%
  select(`3 digit SOC 2020 code`, avg_equal_or_lower_skill_count) %>%
  distinct(`3 digit SOC 2020 code`, .keep_all = TRUE) %>%
  filter(!`3 digit SOC 2020 code` %in% c("511", "911")) # Filter out other Agri codes

# Extract the first 3 characters of C2021_OCC_105_NAME to match with `3 digit SOC 2020 code`
census_sec_emp_selected <- census_sec_emp %>%
  mutate(`3 digit SOC 2020 code` = substr(C2021_OCC_105_NAME, 1, 3)) %>%
  select(`3 digit SOC 2020 code`, C2021_OCC_105) %>%
  distinct(`3 digit SOC 2020 code`, .keep_all = TRUE) # only keep unique values

# Merge the `top_10` list with the selected `C2021_OCC_105` column
top_10_occ_105_group <- top_10_excluding_top_1 %>%
  left_join(census_sec_emp_selected, by = "3 digit SOC 2020 code")
...
```{r}
# Convert both columns to character type before joining
matched_census_data <- census_sec_emp %>%
  mutate(C2021_OCC_105 = as.character(C2021_OCC_105)) %>%
  semi_join(
    top_10_occ_105_group %>% 
      mutate(C2021_OCC_105 = as.character(C2021_OCC_105)), 
    by = "C2021_OCC_105"
  )

# Summarize the OBS_VALUE for each GEOGRAPHY_CODE
top_occ_sum_per_geography <- matched_census_data %>%
  group_by(GEOGRAPHY_CODE) %>%
  summarize(total_obs_value = sum(OBS_VALUE, na.rm = TRUE), .groups = "drop")

# Filter the MSOA boundary and top_occ_sum_per_geography data to include only entries for England
filtered_boundary <- MSOA_21_boundary %>%
  filter(substr(MSOA21CD, 1, 1) == "E")

filtered_top_occ_sum_per_geography <- top_occ_sum_per_geography %>%
  filter(substr(GEOGRAPHY_CODE, 1, 1) == "E")

# Merge top_occ_sum_per_geography with MSOA_21_boundary based on GEOGRAPHY_CODE and MSOA21CD
top_occ_sum_geo_sf <- filtered_top_occ_sum_per_geography %>%
  left_join(filtered_boundary, by = c("GEOGRAPHY_CODE" = "MSOA21CD"))

# Plot the top 10 skill-matching industry total employment by MSOA
ggplot(data = top_occ_sum_geo_sf) +  
  geom_sf(aes(geometry = geometry, fill = ifelse(total_obs_value == 0, NA, total_obs_value)), color = "grey", size = 0.1) +
