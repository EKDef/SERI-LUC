...
MSOA_21_boundary <- st_read("/dbfs/mnt/base/unrestricted/source_ons_open_geography_portal/dataset_middle_s_o_a_2021_ew_bfe/format_SHP_middle_s_o_a_2021_ew_bfe/LATEST_middle_s_o_a_2021_ew_bfe/MSOA_2021_EW_BFC_V7.shp") %>%
  ...
  select(-MSOA21NMW, -BNG_E, -BNG_N, -LAT, -LONG, -GlobalID)
...
# Load NCA boundary file
NCA_boundary <- st_read("/dbfs/mnt/base/unrestricted/source_defra_data_services_platform/dataset_national_character_areas/format_SHP_national_character_areas/LATEST_national_character_areas/National_Character_Areas___Natural_England.shp") %>%
  select(-jcacode, -jcaname, -naid, -area_sqkm, -hotlink, -shape_leng, 
         -shape_le_1, -alt, -blt, -st_area_sh, -st_perimet)
...
# 1.1 Define NCAs of Interest
nca_list_of_interest <- c(
  "The Culm", 
...
# 1.2 Filter NCA boundaries for list of interest
NCA_LoI <- NCA_boundary %>%
  filter(nca_name %in% nca_list_of_interest)  # Adjust column name as needed


# 2.1 Ensure consistent CRS for spatial operations
MSOA_21_boundary <- st_transform(MSOA_21_boundary, st_crs(NCA_boundary))

# 2.2 Perform spatial intersection to identify MSOAs within NCAs
msoa_nca_intersection <- st_intersection(MSOA_21_boundary, NCA_LoI)

# 2.3 Calculate intersection type (fully vs partially within)
msoa_nca_analysis <- msoa_nca_intersection %>%
  mutate(
    # Calculate area of intersection
    intersection_area = st_area(.),
    # Get original MSOA area (need to rejoin with original boundaries)
  ) %>%
  left_join(
    MSOA_21_boundary %>% 
  ...
  mutate(
    # Calculate percentage of MSOA within NCA
    overlap_percentage = as.numeric(intersection_area / original_area) * 100,
    # Classify as fully or partially within
    overlap_type = case_when(
      overlap_percentage >= 95 ~ "Fully Within",
      overlap_percentage >= 10 ~ "Partially Within",
      TRUE ~ "Minimal Overlap"
    )
  ) %>%
  filter(overlap_percentage >= 10)  # Keep only meaningful overlaps


# 3.1 Define the 3 agricultural SOCs of interest
agricultural_socs <- c("8", "60", "96")
# SOC 8 = 121 : Managers and Proprietors in Agriculture Related Services
# SOC 60 = 511 : Agricultural and Related Trades
# SOC 96 = 911 : Elementary Agricultural Occupations  

# 3.2 Calculate agricultural employment by SOC for MSOAs in NCAs
agri_employment_analysis <- census_sec_emp %>%
  filter(
    GEOGRAPHY_CODE %in% msoa_nca_analysis$MSOA21CD,
    C2021_OCC_105 %in% c("0", agricultural_socs)
  ) %>%
  group_by(GEOGRAPHY_CODE) %>%
  summarise(
    total_employment = sum(OBS_VALUE[C2021_OCC_105 == "0"], na.rm = TRUE),
    agri_soc_121 = sum(OBS_VALUE[C2021_OCC_105 == "8"], na.rm = TRUE),
    agri_soc_511 = sum(OBS_VALUE[C2021_OCC_105 == "60"], na.rm = TRUE),
    agri_soc_911 = sum(OBS_VALUE[C2021_OCC_105 == "96"], na.rm = TRUE),
    total_agri_employment = agri_soc_121 + agri_soc_511 + agri_soc_911,
    .groups = "drop"
  ) %>%
  mutate(
    agri_percentage_121 = (agri_soc_121 / total_employment) * 100,
    agri_percentage_511 = (agri_soc_511 / total_employment) * 100,
    agri_percentage_911 = (agri_soc_911 / total_employment) * 100,
    total_agri_percentage = (total_agri_employment / total_employment) * 100
  )

# 4.1 Apply existing skill-matching methodology to NCA MSOAs
skill_matched_employment_nca <- top_occ_sum_per_geography %>%
  filter(GEOGRAPHY_CODE %in% msoa_nca_analysis$MSOA21CD) %>%
  select(
    GEOGRAPHY_CODE, 
    skill_matched_employment = top_occ_obs_value,
    skill_matched_percentage = top_percentage_value
  )
...
# 5.1 Calculate leading  occupations
leading_occupations <- census_sec_emp %>%
  filter(
    GEOGRAPHY_CODE %in% msoa_nca_analysis$MSOA21CD,
    C2021_OCC_105 != "0",  # Exclude total
  ) %>%
  group_by(GEOGRAPHY_CODE) %>%
  arrange(desc(OBS_VALUE)) %>%
  slice_head(n = 5) %>%  # Top 5 occupations
  mutate(rank = row_number()) %>%
  select(GEOGRAPHY_CODE, rank, C2021_OCC_105_NAME, OBS_VALUE) %>%
  pivot_wider(
    names_from = rank,
    values_from = c(C2021_OCC_105_NAME, OBS_VALUE),
    names_prefix = "top_"
  )

# 5.2 Calculate occupation diversity metrics
occupation_diversity <- census_sec_emp %>%
  filter(
    GEOGRAPHY_CODE %in% msoa_nca_analysis$MSOA21CD,
    C2021_OCC_105 != "0" # Exclude total employment
  ) %>%
  group_by(GEOGRAPHY_CODE) %>%
  mutate(
    total_emp = sum(OBS_VALUE, na.rm = TRUE),
    proportion = OBS_VALUE / total_emp
  ) %>%
  summarise(
    # Shannon Diversity Index
    shannon_diversity = -sum(proportion * log(proportion), na.rm = TRUE),
    # Simpson Diversity Index
    simpson_diversity = 1 - sum(proportion^2, na.rm = TRUE),
    # Number of occupation categories with >1% employment
    significant_occupations = sum(proportion > 0.01, na.rm = TRUE),
    .groups = "drop"
  )
```

```{r}
occupation_diversity_improved <- census_sec_emp %>%
  filter(
    GEOGRAPHY_CODE %in% msoa_nca_analysis$MSOA21CD,
    C2021_OCC_105 != "0"
  ) %>%
  ...
  summarise(
    # Traditional diversity indices (will still be high)
    shannon_diversity = -sum(proportion * log(proportion), na.rm = TRUE),
    simpson_diversity = 1 - sum(proportion^2, na.rm = TRUE),
    
    # METRICS FOR CONCENTRATION:
    
    # 1. Effective Number of Occupations (inverse of Simpson)
    effective_occupations = 1 / sum(proportion^2),
    
    # 2. Top N concentration ratios
    top_1_concentration = max(proportion) * 100,
    top_3_concentration = sum(head(sort(proportion, decreasing = TRUE), 3)) * 100,
    top_5_concentration = sum(head(sort(proportion, decreasing = TRUE), 5)) * 100,
    
    # 3. Herfindahl-Hirschman Index (0-10000 scale, higher = more concentrated)
    hhi = sum(proportion^2) * 10000,
    
    # 5. Number of significant occupations (>1%, >2%, >5%)
    occupations_over_1pct = sum(proportion > 0.01),
    occupations_over_2pct = sum(proportion > 0.02),
    occupations_over_5pct = sum(proportion > 0.05),
    
    # 6. Dominance ratio (largest / second largest) - FIXED VERSION
    dominance_ratio = {
      sorted_vals <- sort(proportion, decreasing = TRUE)
  ...
  # 4. Concentration ratio categories - moved outside summarise() to avoid vectorization
  mutate(
    concentration_category = case_when(
      top_1_concentration >= 10 ~ "Highly Concentrated (10%+ in 1 occupation)",
      top_3_concentration >= 20 ~ "Concentrated (20%+ in top 3)",
      top_5_concentration >= 30 ~ "Moderately Concentrated (30%+ in top 5)",
      TRUE ~ "Diverse"
    )
  )
...
final_results <- msoa_nca_analysis %>%
  st_drop_geometry() %>%
  select(MSOA21CD, MSOA21NM, nca_name, overlap_percentage, overlap_type) %>%
  left_join(agri_employment_analysis, by = c("MSOA21CD" = "GEOGRAPHY_CODE")) %>%
  left_join(skill_matched_employment_nca, by = c("MSOA21CD" = "GEOGRAPHY_CODE")) %>%
  left_join(leading_occupations, by = c("MSOA21CD" = "GEOGRAPHY_CODE")) %>%
  left_join(occupation_diversity_improved, by = c("MSOA21CD" = "GEOGRAPHY_CODE")) %>%
  arrange(nca_name, overlap_type, desc(total_agri_percentage))

# 6.2 Export to CSV
write_csv(final_results, "nca_msoa_agricultural_employment_analysis.csv")

# 6.3 Create summary by NCA
nca_summary <- final_results %>%
  group_by(nca_name) %>%
  summarise(
    total_msoas = n(),
    fully_within_count = sum(overlap_type == "Fully Within"),
    partially_within_count = sum(overlap_type == "Partially Within"),
    avg_agri_percentage = mean(total_agri_percentage, na.rm = TRUE),
    avg_skill_matched_percentage = mean(skill_matched_percentage, na.rm = TRUE),
    avg_diversity = mean(top_3_concentration, na.rm = TRUE),
    .groups = "drop"
  )

write_csv(nca_summary, "nca_summary_statistics.csv")
```

```{r}

# Map showing MSOAs within NCAs colored by overlap type
ggplot() +
  geom_sf(data = NCA_boundary, fill = "transparent", color = "grey", linewidth = 1) +
  geom_sf(data = msoa_nca_analysis, aes(fill = overlap_type), alpha = 0.7) +
  geom_sf(data = NCA_LoI, fill = "transparent", color = "red", linewidth = 1.2) +
  scale_fill_manual(values = c("Fully Within" = "darkgreen", 
                              "Partially Within" = "orange")) +
  theme_minimal() +
  labs(title = "MSOAs within Selected NCAs") +
  theme(
    panel.grid.major = element_blank(),
    axis.text = element_blank(),
    axis.ticks = element_blank(),
...
# Save the plot
ggsave("NCA_MSOA_overlap.png", device = "png", width = 25, height = 25, dpi = 200)
```

```{r}

# 7.2 Agricultural employment distribution
ggplot(final_results, aes(x = total_agri_percentage)) +
  geom_histogram(binwidth = 1, alpha = 0.7) +
  facet_wrap(~nca_name) +
  labs(title = "Distribution of Agricultural Employment % by NCA")
```
